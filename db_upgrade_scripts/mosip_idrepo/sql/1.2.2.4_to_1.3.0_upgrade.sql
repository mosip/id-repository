...
CREATE INDEX IF NOT EXISTS idx_uin_auth_lock_hash_type_crdtimes ON idrepo.uin_auth_lock (uin_hash, auth_type_code, cr_dtimes);
CREATE INDEX IF NOT EXISTS idx_uin_auth_lock_covering ON idrepo.uin_auth_lock (uin_hash, auth_type_code, cr_dtimes, status_code, unlock_expiry_datetime);
...
CREATE INDEX IF NOT EXISTS idx_crs_hash_not_deleted ON idrepo.credential_request_status (individual_id_hash) WHERE is_deleted = false;
