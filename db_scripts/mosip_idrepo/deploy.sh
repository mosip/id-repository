### -- ---------------------------------------------------------------------------------------------------------
### -- Script Name		: IDREPO DB Artifacts deploy
### -- Deploy Module 	: MOSIP IDREPO
### -- Purpose    		: To deploy MOSIP IDREPO Database DB Artifacts.       
...
#========================================DB Deployment process begins on IDREPO DB SERVER======================================
...
MASTERCONN=$(PGPASSWORD=$SU_USER_PWD  psql --username=$SU_USER --host=$DB_SERVERIP --port=$DB_PORT --dbname=$DEFAULT_DB_NAME -t -c "select count(1) from pg_roles where rolname IN('idrepouser')";exit; >> $LOG 2>&1)

if [ ${MASTERCONN} == 0 ]
then
    echo `date "+%m/%d/%Y %H:%M:%S"` ": Creating idrepo database user" | tee -a $LOG 2>&1
    ...
    echo `date "+%m/%d/%Y %H:%M:%S"` ": Idrepo user already exist" | tee -a $LOG 2>&1
fi
...
#========================================DB Deployment process completes on IDREPO DB SERVER======================================
