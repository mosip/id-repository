package io.mosip.idrepository.bioextractor.service.helper;
...
public class BioExtractionHelperTest {
	...
	public void testExtractTemplates_Exception() throws Exception {
		...
		iBioProviderApi bioProvider = Mockito.mock(iBioProviderApi.class);
		Mockito.when(bioApiFactory.getBioProvider(Mockito.any(), Mockito.any())).thenReturn(bioProvider);
		Mockito.when(bioProvider.extractTemplate(Mockito.any(), Mockito.any()))
				.thenThrow(new RuntimeException("Technical error"));
		Mockito.when(cbeffUtil.createXML(Mockito.any())).thenReturn("result".getBytes());
