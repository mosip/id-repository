...
public class CredentialStatusManager {
	...
	public String encryptId(String individualId) throws IdRepoAppException {
		...
		return modResult + SPLITTER + new String(securityManager.encryptWithSalt(individualId.getBytes(), CryptoUtil.decodePlainBase64(encryptSalt), uinRefId));
	}
	...
	public String decryptId(String individualId) throws IdRepoAppException {
		...
		return new String(
				securityManager.decryptWithSalt(CryptoUtil.decodePlainBase64(StringUtils.substringAfter(individualId, SPLITTER)), CryptoUtil.decodePlainBase64(encryptSalt.get().getSalt()), uinRefId));
	}
