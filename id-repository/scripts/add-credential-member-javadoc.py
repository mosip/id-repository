#!/usr/bin/env python3
"""Add class-level field and public-method JavaDoc only (not locals inside methods)."""
import re
...
def method_doc(name: str, params, ret: str) -> list:
    ...
    if name.startswith("get") and not params:
        lines.append(f" * Returns {humanize(name[3:])}.")
    elif name.startswith("set") and len(params) == 1:
        lines.append(f" * Sets {humanize(name[3:])}.")
    elif name.startswith("is") and not params and ret == "boolean":
        lines.append(f" * Returns whether {humanize(name[2:])}.")
    else:
    ...
    while i < len(lines):
        ...
        if at_class_body and not has_javadoc_before(out, len(out)):
            ...
            if fm and fm.group(5) not in ("serialVersionUID",):
                ...
                if not fname.isupper() or fname in ("LOGGER",):
