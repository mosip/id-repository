...
public class VidServiceImplTest {
	...
	private IdResponseDTO activatedIdentityResponse() {
	...
	@Before
	public void before() throws IdRepoAppException {
		...
		lenient().when(inProcessIdentityClient.retrieveIdentityByUin(anyString()))
				.thenReturn(activatedIdentityResponse());
	}

	private void mockDeactivatedUin() throws IdRepoAppException {
		IdResponseDTO identityResponse = activatedIdentityResponse();
		identityResponse.getResponse().setStatus("DEACTIVATED");
		when(inProcessIdentityClient.retrieveIdentityByUin(anyString())).thenReturn(identityResponse);
	}

	private void mockUinNotFound() throws IdRepoAppException {
		when(inProcessIdentityClient.retrieveIdentityByUin(anyString()))
	...
	private void mockUinRetrievalFailed() throws IdRepoAppException {
		when(inProcessIdentityClient.retrieveIdentityByUin(anyString()))
				.thenThrow(new RuntimeException("identity retrieval failed"));
	}
	...
	public void testCreateVidUinNotActive() throws IdRepoAppException {
		...
		try {
			...
			assertEquals(String.format(IdRepoErrorConstants.INVALID_UIN.getErrorMessage(),
					IdRepoValidationMessageHelper.invalidRegisteredUinStatus("DEACTIVATED",
							EnvUtil.getUinActiveStatus())),
					e.getErrorText());
		}
	}
	...
	public void testRetrieveUinByVidBlocked() {
		...
		try {
			...
			assertEquals(IdRepoErrorConstants.INVALID_VID.getErrorCode(), e.getErrorCode());
			assertEquals(String.format(IdRepoErrorConstants.INVALID_VID.getErrorMessage(),
					IdRepoValidationMessageHelper.invalidWithAllowed("status", "Blocked",
							List.of(EnvUtil.getVidActiveStatus(), "DRAFT"))),
					e.getErrorText());
		}
	}
	...
	public void testRegenerateVid_InValidStatus() throws IdRepoAppException {
		...
		try {
			...
			assertEquals(IdRepoErrorConstants.INVALID_VID.getErrorCode(), e.getErrorCode());
			assertEquals(String.format(IdRepoErrorConstants.INVALID_VID.getErrorMessage(),
					IdRepoValidationMessageHelper.invalidWithAllowed("status", "INACTIVE",
							Arrays.asList("ACTIVE", "REVOKED", "EXPIRED", "USED", "INVALIDATED", "DEACTIVATED"))),
					e.getErrorText());
		}
	}
