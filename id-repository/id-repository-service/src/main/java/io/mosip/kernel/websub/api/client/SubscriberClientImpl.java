 ...
 * {@code SubscriberClientImpl}.
 * <p>
 * Loaded from {@code id-repository-service} before {@code kernel-websubclient-api.jar}. The
 * published {@code 1.4.0-SNAPSHOT} artifact still calls {@code HttpEntity(Object, MultiValueMap)}
 * with {@link HttpHeaders}, which breaks on Spring 7; commons
 * <a href="https://github.com/mosip/commons/blob/develop/kernel/kernel-websubclient-api/src/main/java/io/mosip/kernel/websub/api/client/SubscriberClientImpl.java">develop</a>
 * uses {@code HttpEntity&lt;MultiValueMap&lt;String, String&gt;&gt;}.
 * </p>
 */
public class SubscriberClientImpl
	...
	public SubscriptionChangeResponse subscribe(SubscriptionChangeRequest subscriptionRequest) {
		...
		if (!isWebsubDbVersionClientBehaviourEnable) {
			map.add(WebSubClientConstants.HUB_CALLBACK, subscriptionRequest.getCallbackURL());
		}
		else {
			...
			headers.set(WebSubClientConstants.SUBSCRIBER_SIGNATURE_HEADER,
					getHmac256(
							failedContentRequest.getTopic() + failedContentRequest.getCallbackURL()
									+ failedContentRequest.getTimestamp() + pageIndex
									+ failedContentRequest.getMessageCount(),
