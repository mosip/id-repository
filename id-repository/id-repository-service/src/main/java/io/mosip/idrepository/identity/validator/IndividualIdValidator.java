...
public class IndividualIdValidator extends BaseIdRepoValidator implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        if (target instanceof IdVidMetadataRequestWrapper wrapper) {
            validateReqTime(wrapper.getRequesttime(), errors);
            if (!errors.hasErrors()) {
                validateVersion(wrapper.getVersion(), errors);
            }
            IdVidMetadataRequestDTO metadataRequest = wrapper.getRequest();
    ...
    private void validateUinOrVidOrRid(String individualId, String idType, Errors errors) throws IdRepoAppException {
        ...
        if (StringUtils.isEmpty(idType)) {
            ...
            try {
                ...
                IdType expectedIdType = IdType.valueOf(idType.toUpperCase());
