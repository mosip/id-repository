...
import static io.mosip.idrepository.core.constant.IdRepoConstants.MASK_FUNCTION_IDENTITY_ATTRIBUTES;
 ...
 */
@Component
@Transactional("vidTransactionManager")
...
@Value("${" + VID_REGENERATE_ALLOWED_STATUS + "}")
	...
	/** The Uin Hash Salt Repo (idmap database). */
	@Autowired
	private VidUinHashSaltRepo uinHashSaltRepo;

	@Autowired
	private VidUinEncryptSaltRepo uinEncryptSaltRepo;

	/** Id. */
	@Resource(name = "vidIdMap")
...
@Autowired(required = true)
	...
	VariableResolverFactory functionFactory;
		...
		/** Identity attribute. */
@Value("${" + MASK_FUNCTION_IDENTITY_ATTRIBUTES + "}")
	...
	/**
	 * Generate vid.
	 * @param vidRequest vid request
	 * @return response wrapper<vid response dto>
	 */
	...
	private Vid generateVid(String uin, String vidType, String vidStatus) throws IdRepoAppException {
		...
		if (Objects.isNull(vidDetails) || vidDetails.isEmpty() || vidDetails.size() < policy.getAllowedInstances()) {
			String vidRefId = UUIDUtils
					.getUUID(UUIDUtils.NAMESPACE_OID, uin + SPLITTER + DateUtils2.getUTCCurrentDateTime()).toString();
	...
	private void checkUinStatus(String uin) throws IdRepoAppException {
		try {
			IdResponseDTO identityResponse = inProcessIdentityClient.retrieveIdentityByUin(uin);
			...
			if (!uinStatus.equals(EnvUtil.getUinActiveStatus())) {
				throw new IdRepoAppException(INVALID_UIN.getErrorCode(),
						String.format(INVALID_UIN.getErrorMessage(), IdRepoValidationMessageHelper
								.invalidRegisteredUinStatus(uinStatus, EnvUtil.getUinActiveStatus())));
			}
		} catch (IdRepoAppException e) {
			if (NO_RECORD_FOUND.getErrorCode().equals(e.getErrorCode())) {
			...
			throw new IdRepoAppException(UIN_RETRIEVAL_FAILED, e);
		}
	}
	...
	private VidResponseDTO updateVidStatus(String vidStatus, Vid vidObject, String decryptedUin, VidPolicy policy)
		...
		if (!(vidStatus.equals(EnvUtil.getVidUnlimitedTxnStatus())
			...
			notify(uin, vidStatus, Collections.singletonList(vidInfo),
	...
	private ResponseWrapper<VidResponseDTO> applyVIDStatus(String uin, String status, String idType,
		...
		if (!vidList.isEmpty()) {
			...
			vidList.forEach(vid -> {
				...
				vid.setUpdatedDTimes(DateUtils2.getUTCCurrentDateTime());
	...
	private void checkRegenerateStatus(String statusCode) throws IdRepoAppException {
		List<String> allowedStatusList = Arrays.stream(allowedStatus.split(",")).map(String::trim)
				.collect(Collectors.toList());
		if (!allowedStatusList.contains(statusCode)) {
			...
			throw new IdRepoAppException(INVALID_VID.getErrorCode(),
					String.format(INVALID_VID.getErrorMessage(), IdRepoValidationMessageHelper
							.invalidWithAllowed("status", statusCode, allowedStatusList)));
		}
	}
	...
	private void checkStatus(String statusCode) throws IdRepoAppException {
		String activeStatus = EnvUtil.getVidActiveStatus();
		if (!(statusCode.equalsIgnoreCase(activeStatus) || statusCode.contentEquals(DRAFT_STATUS))) {
			...
			throw new IdRepoAppException(INVALID_VID.getErrorCode(),
					String.format(INVALID_VID.getErrorMessage(), IdRepoValidationMessageHelper.invalidWithAllowed(
							"status", statusCode, List.of(activeStatus, DRAFT_STATUS))));
		}
	}
	...
	/**
	 * Credential request response consumer.
	 * @param request request
