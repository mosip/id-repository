...
/**
 * Persists encrypted email/phone channel metadata extracted from identity JSON.
 * <p>
 * Used during add/update to store notification channels separately from {@code uin_data}.
 * </p>
 *
 * @see io.mosip.idrepository.identity.entity.ChannelInfo
 */
@Component
...
public class ChannelInfoHelper {
	...
	/** Repository for {@link ChannelInfo} rows. */
	@Autowired
	private ChannelInfoRepo channelInfoRepo;

	/** Salt repository for hash-based channel encryption. */
	@Autowired
	...
	private void updateNoOfRecords(ChannelInfo channelInfo, Integer value) {
		...
		channelInfo.setUpdDTimes(DateUtils2.getUTCCurrentDateTime());
