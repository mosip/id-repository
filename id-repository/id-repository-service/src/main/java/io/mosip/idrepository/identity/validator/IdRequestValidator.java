...
public class IdRequestValidator extends BaseIdRepoValidator implements Validator {
	...
	@Resource(name = "id")
	public void bindIdentityOperationIds(Map<String, String> identityOperationIds) {
		setOperationIds(identityOperationIds);
	}

	@Value("${" + IdRepoConstants.APPLICATION_VERSION + "}")
	public void bindIdentityApplicationVersion(String applicationVersion) {
		setExpectedApplicationVersion(applicationVersion);
	}
	...
	/** Mosip logger. */
	private static final Logger mosipLogger = IdRepoLogger.getLogger(IdRequestValidator.class);
	...
	@Value("#{'${" + IdRepoConstants.KERNEL_MANDATORY_ATTRIBUTES_NEW_REGISTRATION + ":}'.split(',')}")
	/** New registration fields (List<String>). */
	private List<String> newRegistrationFields;

	/** Update uin fields. */
	@Value("#{'${" + IdRepoConstants.KERNEL_MANDATORY_ATTRIBUTES_UPDATE_UIN + ":}'.split(',')}")
	/** Update uin fields (List<String>). */
	private List<String> updateUinFields;

	/** Status from {@code mosip.idrepo.identity.uin-status} (config server). */
	@Resource(name = "uinStatus")
	/** Uin status (List<String>). */
	...
	@Value("${" + IdRepoConstants.AUTH_TYPES_ALLOWED + "}")
	/** Allowed auth types. */
		...
		if (Objects.nonNull(status) && (method.equals(UPDATE) && !this.uinStatus.contains(status))) {
			...
			errors.rejectValue(REQUEST, INVALID_INPUT_PARAMETER.getErrorCode(),
					String.format(INVALID_INPUT_PARAMETER.getErrorMessage(),
							IdRepoValidationMessageHelper.invalidWithAllowed(STATUS_FIELD, status, uinStatus)));
		}
	}
	...
	/**
	 * Validates identity request payload for create or update operations.
	 *
	 * @param request request body object (typically {@link RequestDTO})
	 * @param errors  Spring validation errors binding result
	 * @param method  operation name ({@code create} or {@code update})
	 */
	...
	/**
	 * Validate type and extraction formats.
	 * @param type type
	...
	/**
	 * Validate auth types.
	 * @param authTypeStatusList auth type status list
	 */
